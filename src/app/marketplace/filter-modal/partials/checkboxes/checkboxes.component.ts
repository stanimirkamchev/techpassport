...
export class CheckboxesComponent implements OnInit, OnChanges {
  ...
  ngOnInit(): void {
    ...
    this.copiedDataItems = this.copiedData(this.data);
  ...
  ngOnChanges(changes: SimpleChanges) {
    const change = changes.outputFilteredData;
    if (change && !change.firstChange) {
      if (this.searchedColumn === 'productTags') {
        console.log(this.searchedColumn, this.selectedItems, this.data);
      }
      const newSelected = change.currentValue[this.searchedColumn] || [];
      this.selectedItems = this.selectedItems.map((item) => {
        const selected = newSelected.find((it) => it.name === item.name);
        return selected ? selected : { ...item, isChecked: false };
      });
    }
  }
